...
Aliases"""
    ...
    async def pikachu(self, ctx):
      ...
      embed.set_footer(name="Made By K4MI ⸸#8166", icon_url="")
    ...
    async def roleall(self, ctx, *, role: discord.Role):
        ...
        await ctx.message.add_reaction("<:greentick:1061709816017453117>")
        num = 0
        failed = 0
        for user in list(ctx.guild.members):
            try:
                ...
                failed += 1
        await ctx.send(embed=discord.Embed(title="roleall", description="<:greentick:1061709816017453117> | Successfully added **`%s`** to **`%s`** users, failed to add it to **`%s`** users" % (role.name, num, failed), color=0x00FFE4))
    ...
    async def jail(self, ctx, member: discord.Member):
        ...
        await jail.send(content=member.mention, embed=discord.Embed(title="jail", description="You have been put in jail please live out your jail sentence until the court lets you free. <:jail:1063560071201693838>", color=0x00FFE4))
