...
class Extra(commands.Cog):
    ...
    async def stats(self, ctx):
        ...
        b = Button(label='Invite Me', style=discord.ButtonStyle.link, url='https://discord.com/oauth2/authorize?client_id=1040194948496109569&scope=bot&permissions=8')
        hacker = Button(label='Support Server', style=discord.ButtonStyle.link, url='https://discord.gg/painarmyop')
        ...
        embed = discord.Embed(color=0x00FFE4,description="[Invite](https://discord.com/oauth2/authorize?client_id=1040194948496109569&scope=bot&permissions=8) ● [Support Server](https://discord.gg/painarmyop)")
 
 
        embed.add_field(name='<:Discord:1043973292614226010> • **Servers**', value=f'```Total: {serverCount} Server```')
        embed.add_field(name='<:users:1044192823215394856> • **Users**', value=f'```Total:{total_members} Users```')
        embed.add_field(name="<:selectCommunityAdmin:1040662465765183498> • **System**", value=f"```RAM: {used_memory}/{total_memory} MB\nCPU: {cpu_used}% used.```")
        embed.add_field(name="<:python:1065600084819071068> • **Python Version**", value=f"```{sys.version}```"),
        embed.add_field(name='<:python:1065600084819071068> • **Discord.py Version**', value=f'```{discord.__version__}```')
        #embed.add_field(name='<a:Clock00:1009461606700302337> • **Uptime**', value=f''+uptime+'')
        embed.add_field(
            name="<a:On_Ping:1044251622601146458> • **Ping**",
            value=f"```{round(self.bot.latency * 1000, 2)}ms```")
        hacker = await self.bot.fetch_user(525861159446904832)
        if hacker in ctx.guild.members:
            a = f'{hacker.mention}'
        else:
            a = f'{hacker}'
        embed.add_field(name='<:developer:1040661849458356234> • **Developers**', value=f"{hacker}")
        ...
        embed.set_footer(text='Thanks For Using Odin',icon_url= "https://cdn.discordapp.com/attachments/1046001142057934939/1062786768753147984/odin.png")
    ...
    async def invite(self, ctx: commands.Context):
        embed = discord.Embed(title="**Invite Link:**", description = "> • [Click Here To Invite Odin To Your Server](https://discord.com/oauth2/authorize?client_id=1040194948496109569&scope=bot&permissions=8)\n> • [Click Here To Join My Support Server](https://discord.gg/painarmyop)", color=0x00FFE4, timestamp=ctx.message.created_at)
...
**Version:** V1
            ...
            name="Links:",
            value=f"""
- [Support](https://discord.gg/painarmyop)
- [Invite](https://discord.com/oauth2/authorize?client_id=1040194948496109569&scope=bot&permissions=8)
- [Kami's YT](https://www.youtube.com/@PNKAMI)
            """,
            inline=True
...
**Threads:** {len(guild.threads)}
    ...
    async def removeemoji(self, ctx, emoji: discord.Emoji):
        await emoji.delete()
        await ctx.send(f"**<:greentick:1061709816017453117> emoji has been deleted.**")
    ...
    async def unbanall(self, ctx):
        button = Button(label="Yes", style=discord.ButtonStyle.green, emoji="<:greentick:1061709816017453117>")
        button1 = Button(label="No", style=discord.ButtonStyle.red, emoji="<:red_tick:1062682706317680642>")
    ...
    async def ping(self, ctx):
        ...
        if ping_ < 25:
            client_emoji = "<:online:1044193241639166012>"
        elif ping_ < 50:
            client_emoji = "<:B1:1062683990911369236>"
        else:
            client_emoji = "<:online:1044193241639166012>"
        if db < 25:
            db_emoji = "<:online:1044193241639166012>"
        elif db < 50:
            db_emoji = "<:B1:1062683990911369236>"
        else:
            db_emoji = "<a:On_Ping:1044251622601146458>"
    ...
    async def _badges(self, ctx, user: Optional[discord.User] = None):
      ...
      if badges == []:
        embed2 = discord.Embed(description="**__BADGES__** <a:dcbadges:1040662464850825337>\n\n`No Badge Available`", color=0x00FFE4)
        embed2.set_author(name=f"Profile For {mem}", icon_url = "https://cdn.discordapp.com/attachments/1046001142057934939/1062786768753147984/odin.png")
        ...
        embed = discord.Embed(description="**__BADGES__** <a:dcbadges:1040662464850825337>\n\n",color=0x00FFE4)
        embed.set_author(name=f"Profile For {mem}",icon_url="https://cdn.discordapp.com/attachments/1046001142057934939/1062786768753147984/odin.png")
